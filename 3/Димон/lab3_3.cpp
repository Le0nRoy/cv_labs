...
{
	//подсчЄт ближайшего рассто€ни€, построение линии между лампой и ближайшим роботом
	...
	for (int i = 0; i < cnts.size(); i++)
	{
		if (cnts[i].size() > mini && cnts[i].size() < maxi)
		{
			...
			if (n == 0)
			{
				//по формуле рассто€ни€ между двум€ точками с известными координатами
...
int main()
{
	...
	cvtColor(input_img, cimg, CV_BGR2HSV);//H-оттенок S-насыщенность V-€ркость
	buff = cimg.clone();
	...
	//lamp
	inRange(cimg, Vec3b(0, 0, 150), Vec3b(35, 12, 255), cimg);//двойной порог
	result_img(65, 110, Vec3b(0, 255, 255), 1);
	
	//red
	cimg = buff.clone();
	kernel = getStructuringElement(CV_SHAPE_RECT, Size(3, 3));//формирование €дра дл€ эродии
	erode(cimg, cimg, kernel, Point(-1, -1), 5);//эроди€
